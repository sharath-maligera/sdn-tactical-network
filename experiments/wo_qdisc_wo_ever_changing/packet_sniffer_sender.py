...
def capture_live_packets(config_dict=None):
    ...
    if config_dict is not None:
        ...
        try:
            cap = pyshark.LiveCapture(interface=config_dict.get('from_interface'),bpf_filter='not arp',only_summaries=False,use_json=False)#bpf_filter='host 192.168.0.2 and not arp',,capture_filter='host 192.168.0.2 and not arp'
