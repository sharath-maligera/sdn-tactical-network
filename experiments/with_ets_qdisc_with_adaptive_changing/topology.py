...
def myNet():
    ...
    # Create switches
    s1 = net.addSwitch('s1', cls = OVSSwitch, protocols='OpenFlow13', listenPort=6634, dpid='0000000000000010')
    s2 = net.addSwitch('s2', cls = OVSSwitch, protocols='OpenFlow13', listenPort=6634, dpid='0000000000000020')
    s3 = net.addSwitch('s3', cls = OVSSwitch, protocols='OpenFlow13', listenPort=6634, dpid='0000000000000030')
    s4 = net.addSwitch('s4', cls = OVSSwitch, protocols='OpenFlow13', listenPort=6634, dpid='0000000000000040')
    ...
    # s1.cmd('/sbin/tc qdisc add dev s1-eth2 root handle 1: htb default 1 && '
    #        '/sbin/tc class add dev s1-eth2 parent 1: classid 1:1 htb rate 762kbit ceil 762kbit burst 762kb && '
    #        '/sbin/tc class add dev s1-eth2 parent 1:1 classid 1:11 htb rate 9.6kbit ceil 9.6kbit burst 10kb && '
    #        '/sbin/tc class add dev s1-eth2 parent 1:1 classid 1:12 htb rate 240kbit ceil 240kbit burst 240kb && '
    #        '/sbin/tc class add dev s1-eth2 parent 1:1 classid 1:13 htb rate 512kbit ceil 512kbit burst 512kb && '
    #        '/sbin/tc filter add dev s1-eth2 parent 1:0 protocol ip prio 1 u32 match ip src 192.168.10.10 match ip protocol 17 0xff flowid 1:1 && '
    #        '/sbin/tc filter add dev s1-eth2 parent 1:1 protocol ip prio 1 u32 match ip dst 192.168.20.10 match ip protocol 17 0xff flowid 1:11 && '
    #        '/sbin/tc filter add dev s1-eth2 parent 1:1 protocol ip prio 1 u32 match ip dst 192.168.30.10 match ip protocol 17 0xff flowid 1:12 && '
    #        '/sbin/tc filter add dev s1-eth2 parent 1:1 protocol ip prio 1 u32 match ip dst 192.168.40.10 match ip protocol 17 0xff flowid 1:13'
    #        )
    ...
    r1.cmd('/sbin/tc qdisc add dev r1-eth3 root handle 1: htb default 1 && '
           '/sbin/tc class add dev r1-eth3 parent 1: classid 1:1 htb rate 30kbit ceil 30kbit burst 512kb && '
    ...
    stdin, stdout, stderr = ssh_to_ctrl_1.exec_command('python ~/sdn-tactical-network/rest_app/qos_app/qos_rest_1.py')
    print "STDOUT:\n%s\n\nSTDERR:\n%s\n" % (stdout.read(), stderr.read())
    ...
    stdin, stdout, stderr = ssh_to_ctrl_1.exec_command('python ~/sdn-tactical-network/rest_app/qos_app/qos_rest_2.py')
    print "STDOUT:\n%s\n\nSTDERR:\n%s\n" % (stdout.read(), stderr.read())

    # s1.cmd('/sbin/tc class change dev s1-eth2 parent 1: classid 1:11 htb rate 1.2kbit')
    # s1.cmd('/sbin/tc class change dev s1-eth2 parent 1: classid 1:12 htb rate 30kbit')
    # s1.cmd('/sbin/tc class change dev s1-eth2 parent 1: classid 1:13 htb rate 64kbit')
    ...
    stdin, stdout, stderr = ssh_to_ctrl_1.exec_command('python ~/sdn-tactical-network/rest_app/qos_app/qos_rest_3.py')
    print "STDOUT:\n%s\n\nSTDERR:\n%s\n" % (stdout.read(), stderr.read())

    # s1.cmd('/sbin/tc class change dev s1-eth2 parent 1: classid 1:11 htb rate 2.4kbit')
    # s1.cmd('/sbin/tc class change dev s1-eth2 parent 1: classid 1:12 htb rate 60kbit')
    # s1.cmd('/sbin/tc class change dev s1-eth2 parent 1: classid 1:13 htb rate 128kbit')
    ...
    stdin, stdout, stderr = ssh_to_ctrl_1.exec_command('python ~/sdn-tactical-network/rest_app/qos_app/qos_rest_4.py')
    print "STDOUT:\n%s\n\nSTDERR:\n%s\n" % (stdout.read(), stderr.read())

    # s1.cmd('/sbin/tc class change dev s1-eth2 parent 1: classid 1:11 htb rate 4.8kbit')
    # s1.cmd('/sbin/tc class change dev s1-eth2 parent 1: classid 1:12 htb rate 120kbit')
    # s1.cmd('/sbin/tc class change dev s1-eth2 parent 1: classid 1:13 htb rate 256kbit')

    sleep(40)

    r1.cmd('/sbin/tc class change dev r1-eth1 parent 1: classid 1:1 htb rate 9.6kbit')
    info("Setting vhf data rate to 0.6kbps \n\n\n")
    r1.cmd('/sbin/tc class change dev r1-eth2 parent 1: classid 1:1 htb rate 256kbit')
    info("Setting uhf data rate to 15kbps \n\n\n")
    r1.cmd('/sbin/tc class change dev r1-eth3 parent 1: classid 1:1 htb rate 512kbit')
    info("Setting SatComm data rate to 32kbps \n\n\n")

    stdin, stdout, stderr = ssh_to_ctrl_1.exec_command('python ~/sdn-tactical-network/rest_app/qos_app/qos_rest_5.py')
    print "STDOUT:\n%s\n\nSTDERR:\n%s\n" % (stdout.read(), stderr.read())

    # s1.cmd('/sbin/tc class change dev s1-eth2 parent 1: classid 1:11 htb rate 9.6kbit')
    # s1.cmd('/sbin/tc class change dev s1-eth2 parent 1: classid 1:12 htb rate 256kbit')
    # s1.cmd('/sbin/tc class change dev s1-eth2 parent 1: classid 1:13 htb rate 512kbit')
