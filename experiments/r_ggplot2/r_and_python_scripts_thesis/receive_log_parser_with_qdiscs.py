...
def end_to_end_delay_with_qdisc(receive_events_with_qdisc=None):
    if receive_events_with_qdisc is not None:
        ...
        df_with_qdisc.to_csv(os.path.join(os.path.dirname(__file__), 'plot_wo_qdisc_uhf_exp_3.csv'), index=False, header=True)
...
def plot_with_qdiscs():
    ...
    receive_log_with_qdisc = os.path.join(experiments_folder, os.path.join('data', os.path.join('without_qdiscs','receive_log_uhf_exp_3.txt')))
