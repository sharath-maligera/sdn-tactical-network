...
def end_to_end_delay_with_qdisc(receive_events_with_qdisc=None):
    if receive_events_with_qdisc is not None:
        ...
        df_with_qdisc.to_csv(os.path.join(os.path.dirname(__file__), 'data_with_ets_qdiscs_0_6_kbps_wo_timeout.csv'), index=False, header=True)
...
def plot_with_qdiscs():
    ...
    receive_log_with_qdisc = os.path.join(experiments_folder, os.path.join('data', os.path.join('with_shaping_and_ets_scheduling','receive_log_0_6_kbps_wo_timeout.txt')))
