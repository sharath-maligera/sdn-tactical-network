...
def end_to_end_bandwidth_with_and_without_qdisc(receive_events_wo_qdisc=None, receive_events_with_qdisc=None):
    if None not in (receive_events_wo_qdisc, receive_events_with_qdisc):
        ...
        for event in receive_events_wo_qdisc:
            if len(event) >= 9:
                ...
                event_wo_qdisc_dict['flow_id'].append(flow_id)
                event_wo_qdisc_dict['packet_seq_no'].append(packet_seq_id)
        ...
        for event in receive_events_with_qdisc:
            if len(event) >= 9:
                ...
                event_with_qdisc_dict['flow_id'].append(flow_id)
                event_with_qdisc_dict['packet_seq_no'].append(packet_seq_id)
        ...
        wo_qdisc_start_time = data_wo_qdisc["packet_sent"].min()
        wo_qdisc_duration_timestamp = [datetime.datetime.combine(datetime.date.today(), t1) - datetime.datetime.combine(datetime.date.today(), wo_qdisc_start_time) for t1, t2 in zip(data_wo_qdisc["packet_sent"], data_wo_qdisc["packet_sent"]) if not (t1!=t1 or t2!=t2)]
        wo_qdisc_duration_in_secs = [duration.total_seconds() for duration in wo_qdisc_duration_timestamp]
        data_wo_qdisc.insert(8, "duration_in_secs", wo_qdisc_duration_in_secs, True)
        ...
        data_wo_qdisc.insert(9, "packet_delay_in_secs", wo_qdisc_packet_delay_in_secs, True)
        data_wo_qdisc['label'] = 'with_out_qdisc'

        with_qdisc_start_time = data_with_qdisc["packet_sent"].min()
        with_qdisc_duration_timestamp = [datetime.datetime.combine(datetime.date.today(), t1) - datetime.datetime.combine(datetime.date.today(), with_qdisc_start_time) for t1, t2 in zip(data_with_qdisc["packet_sent"], data_with_qdisc["packet_sent"]) if not (t1 != t1 or t2 != t2)]
        with_qdisc_duration_in_secs = [duration.total_seconds() for duration in with_qdisc_duration_timestamp]
        data_with_qdisc.insert(8, "duration_in_secs", with_qdisc_duration_in_secs, True)
        ...
        data_with_qdisc.insert(9, "packet_delay_in_secs", with_qdisc_packet_delay_in_secs, True)
        data_with_qdisc['label'] = 'with_qdisc'

        df_wo_qdisc = pd.DataFrame(data_wo_qdisc, columns=['duration_in_secs', 'packet_delay_in_secs', 'flow_id', 'label'])
        df_wo_qdisc.to_csv(os.path.join(os.path.dirname(__file__), 'data_wo_qdisc.csv'), index=False, header=True)

        df_with_qdisc = pd.DataFrame(data_with_qdisc,columns=['duration_in_secs', 'packet_delay_in_secs', 'flow_id', 'label'])
        df_with_qdisc.to_csv(os.path.join(os.path.dirname(__file__), 'data_with_qdisc.csv'), index=False, header=True)

        df_with_and_without_qdisc = pd.concat([df_wo_qdisc, df_with_qdisc], ignore_index=True)
        df_with_and_without_qdisc.to_csv(os.path.join(os.path.dirname(__file__), 'data_with_and_without_qdisc.csv'), index=False, header=True)
        pass
...
def plot_with_and_without_qdisc():
    ...
    receive_log_wo_qdisc = os.path.join(experiments_folder, os.path.join('wo_qdisc_wo_ever_changing','receive_log.txt'))
    receive_log_with_qdisc = os.path.join(experiments_folder, os.path.join('with_qdisc_wo_ever_changing', 'receive_log.txt'))
    ...
    end_to_end_bandwidth_with_and_without_qdisc(receive_events_wo_qdisc=receive_events_wo_qdisc, receive_events_with_qdisc=receive_events_with_qdisc)
