...
def end_to_end_delay_with_and_without_qdisc(receive_events_wo_qdisc=None, receive_events_with_qdisc=None):
    if None not in (receive_events_wo_qdisc, receive_events_with_qdisc):
        ...
        # wo_qdisc_duration_timestamp = [datetime.datetime.combine(datetime.date.today(), t1) - datetime.datetime.combine(datetime.date.today(), wo_qdisc_start_time) for t1, t2 in zip(data_wo_qdisc["packet_received"], data_wo_qdisc["packet_received"]) if not (t1!=t1 or t2!=t2)]
        ...
        # with_qdisc_start_time = data_with_qdisc["packet_received"].min()
        # with_qdisc_duration_timestamp = [datetime.datetime.combine(datetime.date.today(), t1) - datetime.datetime.combine(datetime.date.today(), with_qdisc_start_time) for t1, t2 in zip(data_with_qdisc["packet_received"], data_with_qdisc["packet_received"]) if not (t1 != t1 or t2 != t2)]
        ...
        df_wo_qdisc = pd.DataFrame(data_wo_qdisc, columns=['packet_seq_no', 'packet_delay_in_secs', 'flow_id', 'label'])
        df_wo_qdisc.to_csv(os.path.join(os.path.dirname(__file__), 'data_wo_qdisc.csv'), index=False, header=True)

        df_with_qdisc = pd.DataFrame(data_with_qdisc,columns=['packet_seq_no', 'packet_delay_in_secs', 'flow_id', 'label'])
...
def plot_with_and_without_qdisc():
    ...
    end_to_end_delay_with_and_without_qdisc(receive_events_wo_qdisc=receive_events_wo_qdisc, receive_events_with_qdisc=receive_events_with_qdisc)
