...
def view_qdisc_meter_log():
    ...
    qdisc_log = os.path.join(experiments_folder, os.path.join('data', os.path.join('with_shaping_and_ets_scheduling_no_timeout','qdisc_log_9_6_240_kbps.csv')))
    df_qdisc = pd.read_csv(qdisc_log)
    meter_log = os.path.join(experiments_folder, os.path.join('data',os.path.join('with_shaping_and_ets_scheduling_no_timeout','meter_log_9_6_240_kbps.csv')))
...
def parse_qdisc_meter_log():
    ...
    # df_qdisc.to_csv(os.path.join(os.path.dirname(__file__), 'plot_qdisc_log_9_6_240_kbps.csv'), index=False, header=True)
    ...
    df_meter = pd.DataFrame(df_meter, columns=['duration_in_secs', 'meter_id', 'packets', 'bytes'])
